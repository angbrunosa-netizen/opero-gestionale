...
const CatalogSelectionBlock = ({
    ...
    const loadSelezione = async () => {
        try {
            setLoading(true);
            const res = await api.get(`/public/catalog/selezioni/${selezione_id}`);
            if (res.data.success) {
                setSelezione(res.data.data.selezione);
                setProdotti(res.data.data.articoli || []);
            }
        } catch (error) {
    ...
    const getCardStyle = (productId) => {
        const baseStyle = {
            backgroundColor: config.colore_sfondo_card || '#ffffff',
