...
const EntitySelector = ({ listItems, setListItems, listType, idDittaDestinataria }) => {
  ...
  // --- MODIFICA CHIAVE: Funzione di caricamento prezzi con fallback al Listino 1 ---
  const loadPrezzi = async (articoloId, clienteId) => {
    ...
    try {
      ...
      // LOGICA PRINCIPALE: Prova a ottenere i prezzi specifici del cliente
      if (clienteId) {
        try {
          const responsePrezzi = await api.get(`/liste/prezzi/${articoloId}/${clienteId}`);
          
          // Se l'API restituisce prezzi calcolati validi, usali
          if (responsePrezzi.data && responsePrezzi.data.prezziCalcolati) {
            ...
            // Se il cliente non ha listini configurati, apri la modale
          ...
          // Se c'è un errore con i prezzi del cliente (es. cliente non trovato),
          // passa silenziosamente alla logica di fallback.
  ...
  useEffect(() => {
    ...
    // Resetta la cache dei prezzi del cliente
    setPrezziCliente({});
  }, [idDittaDestinataria]);
  ...
  const handleAddItem = () => {
    ...
    const prezzoUnitario = prezziCliente.prezzo_cessione || 0;
    const prezzoPubblico = prezziCliente.prezzo_pubblico || 0;
    ...
    <>
      <div className="entity-selector p-4 bg-white rounded shadow mb-4">
        <h3 className="text-lg font-medium mb-4">
          ...
          {idDittaDestinataria && listType === 'ORDINE_VEND' && (
            <span className="text-sm text-gray-500 ml-2">
              (Prezzi per cliente {idDittaDestinataria})
            </span>
          )}
        </h3>
        ...
        <div className="flex mb-4">
          ...
          <button onClick={handleAddItem} disabled={!selectedItem} className="px-4 py-2 bg-blue-600 text-white rounded disabled:bg-gray-400">
            Aggiungi
          </button>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Q.tà</th>
                {(listType === 'ORDINE_ACQ' || listType === 'ORDINE_VEND') && (
                  <>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pr. Cessione</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Pr. Pubblico</th>
                    {user.livello > 60 && <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Listino</th>}
                  </>
                )}
              ...
              {listItems.map((item, index) => (
                <tr key={index}>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div>
                      <div className="text-sm font-medium text-gray-900">{item.articolo.codice_articolo || item.articolo.codice}</div>
                      <div className="text-sm text-gray-500">{item.articolo.descrizione}</div>
                      {item.listino_cessione_usato && <div className="text-xs text-blue-600 font-medium">C: {item.listino_cessione_usato}</div>}
                      {item.listino_pubblico_usato && <div className="text-xs text-green-600 font-medium">P: {item.listino_pubblico_usato}</div>}
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap"><input type="number" min="0" step="0.01" value={item.qta} onChange={(e) => handleUpdateItem(index, 'qta', parseFloat(e.target.value) || 0)} className="w-20 p-1 border border-gray-300 rounded" /></td>
                  {(listType === 'ORDINE_ACQ' || listType === 'ORDINE_VEND') && (
                    <>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <span className="text-gray-500 text-sm">€</span>
                          <input type="number" min="0" step="0.01" value={item.prezzo_unitario?.toFixed(2) || "0.00"} onChange={(e) => handlePrezzoChange(index, 'unitario', e.target.value)} disabled={user.livello <= 60} className={`w-24 p-1 border rounded text-sm ${user.livello <= 60 ? 'border-gray-200 bg-gray-50' : 'border-gray-300'}`} />
                          {user.livello <= 60 && <span className="text-xs text-gray-500" title="Modifica prezzi solo per utenti livello > 60">🔒</span>}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <span className="text-gray-500 text-sm">€</span>
                          <input type="number" min="0" step="0.01" value={item.prezzo_pubblico?.toFixed(2) || "0.00"} onChange={(e) => handlePrezzoChange(index, 'pubblico', e.target.value)} disabled={user.livello <= 60} className={`w-24 p-1 border rounded text-sm ${user.livello <= 60 ? 'border-gray-200 bg-gray-50' : 'border-gray-300'}`} />
                          {user.livello <= 60 && <span className="text-xs text-gray-500" title="Modifica prezzi solo per utenti livello > 60">🔒</span>}
                        </div>
                      </td>
                      {user.livello > 60 && (
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button onClick={() => { setCurrentItem(item); api.get(`/liste/prezzi-disponibili/${item.id_articolo}`).then(response => { setListiniDisponibili(response.data.listini_disponibili); setListinoModalOpen(true); }).catch(error => { console.error('Errore caricamento listini:', error); alert('Impossibile caricare i listini disponibili'); }); }} className="px-3 py-1 bg-indigo-600 text-white text-sm rounded hover:bg-indigo-700">
                            📋 Listini
                          </button>
                        </td>
                      )}
                  ...
                  <td className="px-6 py-4 whitespace-nowrap"><input type="text" value={item.note || ''} onChange={(e) => handleUpdateItem(index, 'note', e.target.value)} className="w-full p-1 border border-gray-300 rounded" /></td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium"><button onClick={() => handleRemoveItem(index)} className="text-red-600 hover:text-red-900">Rimuovi</button></td>
                </tr>
              ))}
            </tbody>
          </table>
          {listItems.length === 0 && <div className="text-center py-4 text-gray-500">Nessun articolo aggiunto. Usa il campo di ricerca sopra per aggiungere articoli.</div>}
        </div>
      </div>

      <ListinoSelector isOpen={listinoModalOpen} onClose={() => { setListinoModalOpen(false); setCurrentItem(null); }} articolo={currentItem?.articolo} listiniDisponibili={listiniDisponibili} currentListinoCessione={currentItem?.listino_cessione_usato} currentListinoPubblico={currentItem?.listino_pubblico_usato} onConfirm={handleListinoConfirm} userLevel={user?.livello || 0} />
    </>
  );
};
