...
exports.up = async function(knex) {
  ...
  await knex.schema.createTable('va_clienti_anagrafica', function(table) {
    ...
    // Referenti interni (collegati a utenti) - RIMOSSO .unsigned() PER CORRISPONDERE A utenti.id
    table.integer('id_referente').nullable().references('id').inTable('utenti').onDelete('SET NULL');
    table.integer('id_referente_allert').nullable().references('id').inTable('utenti').onDelete('SET NULL');
    table.integer('id_referente_ppa').nullable().references('id').inTable('utenti').onDelete('SET NULL');
    table.integer('id_agente').nullable().references('id').inTable('utenti').onDelete('SET NULL');
...
exports.down = async function(knex) {
  ...
  // Utilizzando dropTableIfExists, la funzione è sicura anche se la migrazione 'up' è fallita a metà.
  await knex.schema.dropTableIfExists('va_punti_consegna');
  await knex.schema.dropTableIfExists('va_clienti_anagrafica');
  console.log('Rollback tabelle anagrafica clienti e punti consegna completato.');
};
