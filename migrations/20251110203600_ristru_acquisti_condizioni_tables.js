...
exports.up = function(knex) {
  return knex.schema
    ...
    .createTable('ac_sconti_dettaglio', function(table) {
        table.increments('id').primary();
        table.integer('id_riga_condizione').unsigned().notNullable().references('id').inTable('ac_condizioni_righe').onDelete('CASCADE');
        ...
        table.index('id_riga_condizione');
    });
};
...
exports.down = function(knex) {
  // Il rollback ripristina la struttura precedente (se necessario)
  // Per semplicità, qui eliminiamo solo la nuova struttura.
  return knex.schema
    .dropTableIfExists('ac_sconti_dettaglio')
