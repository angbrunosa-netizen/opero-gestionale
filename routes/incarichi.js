// #####################################################################
// # Rotte per la Gestione Incarichi - v1.3 (con Creatore Incarico)
// # File: opero/routes/incarichi.js
// #####################################################################
...
// --- GET (Incarichi futuri per l'utente loggato) ---
router.get('/miei-futuri', async (req, res) => {
    ...
    try {
        const query = `
            ...
            LIMIT 5
        `;
        const [incarichi] = await dbPool.query(query, [utenteId, oggi]);
        res.json({ success: true, data: incarichi });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Errore interno del server.' });
    }
});

// --- GET (Tutti gli incarichi futuri della ditta per Admin) ---
router.get('/ditta/futuri', checkRole([1, 2]), async (req, res) => {
    ...
    try {
        ...
        `;
        const [incarichi] = await dbPool.query(query, [dittaId, oggi]);
        res.json({ success: true, data: incarichi });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Errore interno del server.' });
    }
...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        `;
        const [incarichi] = await dbPool.query(query, [dittaId, startDate, endDate]);
        res.json({ success: true, data: incarichi });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Errore interno del server.' });
    }
...
router.post('/', async (req, res) => {
    ...
    try {
        const query = 'INSERT INTO incarichi (id_ditta, titolo, descrizione, data_scadenza, id_utente_creatore, id_utente_assegnato) VALUES (?, ?, ?, ?, ?, ?)';
...
router.patch('/:id/stato', async (req, res) => {
    ...
    try {
        const [result] = await dbPool.query(
            'UPDATE incarichi SET stato = ? WHERE id = ? AND id_ditta = ? AND id_utente_assegnato = ?',
